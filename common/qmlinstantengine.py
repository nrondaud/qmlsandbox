...
class QmlInstantEngine(QQmlApplicationEngine):
    """
    QmlInstantEngine is an utility class helping developing QML a>pplications.
    ...
    def load(self, sourceFile):
        ...
        super(QmlInstantEngine, self).load(QUrl.fromLocalFile(sourceFile))
    ...
    def addFilesFromDirectory(self, dirname, recursive=False):
        ...
        if not os.path.isdir(dirname):
            raise RuntimeError(
                "addFilesFromDirectory : %s is not a valid directory." % dirname
            )
        if recursive:
