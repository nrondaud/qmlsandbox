...
class QObjectListModel(QtCore.QAbstractListModel):
    ...
    def extend(self, iterable):
        ...
        self.beginInsertRows(
            QtCore.QModelIndex(), self.size(), self.size() + len(iterable) - 1
        )
    ...
    def move(self, fromIndex, toIndex):
        ...
        if not self.beginMoveRows(
            QtCore.QModelIndex(), fromIndex, fromIndex, QtCore.QModelIndex(), value
        ):
            return
    ...
    def _referenceItem(self, item):
        ...
        if key in self._objectByKey:
            raise ValueError(
                "Object key {}: {} is not unique".format(self._keyAttrName, key)
            )
